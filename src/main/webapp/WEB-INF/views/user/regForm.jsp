...
$(document).ready(function() {
	...
	$("#userId").on("input", function() {
        ...
        if (userId.length === 0) {
            $("#warningText").text("아이디를 입력하세요.").css("color", "red");
            
        } else if (fn_validateEmpty(userId)) {
            $("#warningText").text("아이디는 공백을 포함할수 없습니다.").css("color", "red")
        
        } else if (!fn_validateIdPwd(userId)) {
            $("#warningText").text("아이디는 4 ~ 12자 영문 대소문자와 숫자로만 입력가능합니다.").css("color", "red");
        ...
        $.ajax({
            ...
            success : function(response){
                if (response.code === 200) {
                    $("#warningText").text("사용 가능한 아이디입니다.").css("color", "blue");
                    $("#userId").attr("readonly", true);
                    $("#id-btn").prop("disabled", true);
                   
                } else if (response.code === 409) {
                    $("#warningText").text("이미 사용중인 아이디입니다.").css("color", "red");
                    $("#userId").focus();
                    
                } else if (response.code === 400) {
                    $("#warningText").text("아이디가 정상적으로 입력되지 않았습니다.").css("color", "red");
                    $("#userId").focus();
		...
		<div class="signup-container">
			...
			<form action="/user/userProc.jsp" method="POST" id="regForm" name="regForm">
				<div class="input-group">
                    ...
                    <button type="button" class="id-btn" id="id-btn">중복 체크</button>
				</div>
				<div class="input-group">
                ...
                <div class="input-group" style="display: flex; align-items: center;">
                    ...
                    <img id="imagePreview" style="display:none; max-width: 200px; max-height: 200px;">
                </div>
